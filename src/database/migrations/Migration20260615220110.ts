...
export class Migration20260615220110 extends Migration {
  ...
  override up(): void | Promise<void> {
    this.addSql(`create table "stock_reservations" ("id" uuid not null, "created_at" timestamptz not null, "updated_at" timestamptz null, "is_deleted" boolean not null default false, "deleted_at" timestamptz null, "variant_id" uuid not null, "warehouse_id" uuid not null, "quantity" numeric(10,3) not null, "reference_id" uuid null, "reference_type" varchar(100) null, "expires_at" timestamptz null, "is_active" boolean not null default true, primary key ("id"));`);
    ...
    this.addSql(`create table "inventory_transactions" ("id" uuid not null, "variant_id" uuid not null, "warehouse_id" uuid not null, "type" text not null, "quantity" numeric(10,3) not null, "reference_id" uuid null, "reference_type" varchar(100) null, "notes" text null, "actor_id" uuid null, "created_at" timestamptz not null, primary key ("id"));`);
    ...
    this.addSql(`drop index "product_variants_specs_gin_idx";`);

    this.addSql(`drop index "audit_logs_entity_type_entity_id_index";`);
    this.addSql(`create index "idx_audit_entity" on "audit_logs" ("entity_id");`);
    this.addSql(`alter index "audit_logs_actor_id_index" rename to "idx_audit_actor";`);
  }
  ...
  override down(): void | Promise<void> {
    ...
    this.addSql(`drop index "idx_audit_entity";`);
    this.addSql(`create index "audit_logs_entity_type_entity_id_index" on "audit_logs" ("entity_type", "entity_id");`);
    this.addSql(`alter index "idx_audit_actor" rename to "audit_logs_actor_id_index";`);

    this.addSql(`create index "product_variants_specs_gin_idx" on "product_variants" ("specs");`);
  }
